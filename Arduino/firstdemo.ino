...
//Race through LEDs in red
void loop() {


for(int k=0; k<NUMLEDS; k++){
  leds.setPixelColor(k, leds.Color(0,0,0)); // blank the array
  leds.show();
  delay(wait);
}

for(int k=0; k<NUMLEDS; k++){
  leds.setPixelColor((k-1)%NUMLEDS, leds.Color(255,0,0));
  leds.setPixelColor(k,leds.Color(255,0,0));
  leds.setPixelColor((k+1)%NUMLEDS, leds.Color(255,0,0));
  leds.show();
  delay(wait);
}
  ...
  for(int i=0; i<255; i++) {            // Count up color units from 0 to 255
    for(int k=0; k < NUMLEDS; k++){
    leds.setPixelColor(k, leds.Color(i, 0, 0));
  }// Color mix for LED0
  ...
  for(int i=0; i<255; i++) {            // Loop through next color change
   for(int k=0; k<NUMLEDS; k++){
    leds.setPixelColor(k, leds.Color(0, i, 0)); 
   }
  ...
  for(int i=0; i<255; i++) {            // Loop through next color change
    for(int k=0; k<NUMLEDS; k++){
    leds.setPixelColor(k, leds.Color(0, 0, i));
